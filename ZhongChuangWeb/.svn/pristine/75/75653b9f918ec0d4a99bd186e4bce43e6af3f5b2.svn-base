				...
				<div class="panel panel-default">
					...
					<div class="panel-body">	
						...
						<table	class="table table-hover table-striped table-bordered table-responsive">
							...
							<tbody>
								<c:forEach var="project" items="${selectProjectList}" >
									<tr>
										...
										<c:choose>
													<c:when test="${project.adminState==0 and project.fileState==1}">
														<a class="btn btn-warning btn-sm" id="checkProgram" onclick="return check_Program()" href ="${pageContext.request.contextPath}/servlet/admin/AdminStateServlet?userNo=${project.userNo}&findFlag=1&programName=${project.projectName}&adminState=1"><i class="glyphicon glyphicon-check"></i></a>											 		
													</c:when>
													<c:when test="${project.adminState==0 and project.fileState==0}">
												 	...
												 	<c:otherwise>
												 		<a  class="btn btn-danger btn-sm " href="${pageContext.request.contextPath}/servlet/admin/AdminStateServlet?findFlag=1&userNo=${project.userNo}&programName=${project.projectName}&adminState=0"><i class="">已批阅</i></a>
												 	</c:otherwise>
										</c:choose>
										...
										<c:choose>
													<c:when test="${project.changeState==0}">
														<a class="btn btn-success btn-sm" id="quanxian" onclick="return quanxian_Program()" href ="${pageContext.request.contextPath}/servlet/admin/ChangeStateServlet?findFlag=1&userNo=${project.userNo}&programName=${project.projectName}&changeState=1"><i class="glyphicon glyphicon-wrench"></i></a>
													</c:when>
												 	<c:otherwise>
												 		<a  class="btn btn-success btn-sm" href="${pageContext.request.contextPath}/servlet/admin/ChangeStateServlet?findFlag=1&userNo=${project.userNo}&programName=${project.projectName}&changeState=0&currentPage=${adminPageBean.currentPage}&pageSize=${adminPageBean.pageSize}"><i>可修改</i></a>
												 	</c:otherwise>
										</c:choose>
										</td> --%>
										<td class="col-sm-1" style="text-align:center;">
										<a class="btn btn-danger btn-sm" onclick="return delete_Program()" href ="${pageContext.request.contextPath}/servlet/admin/DeleteProgramServlet?findFlag=1&userNo=${project.userNo}&programName=${project.projectName}&deleteFlag=1&currentPage=${adminPageBean.currentPage}&pageSize=${adminPageBean.pageSize}"><i class="glyphicon glyphicon-remove"></i></a>
										</td>
										<td class="col-sm-1" style="text-align:center;">
										<c:choose>
											<c:when test="${project.adminState==1}">
												<a class="btn btn-info btn-sm" id="changeProjectIdButton" href="javascript:void(0)" onclick="return changeFind_projectId(this)"><i class="glyphicon">提交</i></a>
											</c:when>
											<c:otherwise>
