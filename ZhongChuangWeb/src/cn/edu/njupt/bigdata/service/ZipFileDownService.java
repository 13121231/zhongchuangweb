...
public class ZipFileDownService {
	/**
	* 把接受的全部文件打成压缩包
	* 
	* @param List
	...
	* @param returnFileName 压缩包文件名
	* @param isDrop 是否删除原文件:true删除、false不删除
	* @param encoding 编码；默认：GBK
	* @throws Exception
	*/
	
	public static void downLoadFiles(HttpServletResponse response, String id, String filePath,String returnFileName,Boolean isDrop,String encoding) throws Exception {
	    File zipFile = new File(generateZip(id,filePath, "home/ownLoad/"+returnFileName,isDrop));
	    if (zipFile.exists()) {
	        response.setContentType("application/octet-stream");
	        String headerDisp = "attachment;filename=" + zipFile.getName() + ".zip";
	        headerDisp = new String(headerDisp.getBytes("".equals(encoding)?"UTF-8":encoding), "iso8859-1");
	...
	*/
	...
	public static String generateZip(String id,String path, String returnPath,Boolean isDrop)throws Exception {
	    ...
	    if (!outFile.exists() || !outFile.isFile() ) {
	        if(!outFile.isFile()){
	            outFile=new File(outFile.getPath()+"/"+id);
	        }
