				 ...
				 if(item.isFormField()) {
					...
					//吴道亮  2016.05.23↑ 删除
					 ...
					 if(fileName.split("\\.")[fileName.split("\\.").length - 1].equals("xls")||fileName.split("\\.")[fileName.split("\\.").length - 1].equals("xlsx")) {
					  realFileName = programName + "-" + author  + "." + fileName.split("\\.")[fileName.split("\\.").length - 1];
					 }else{
					  realFileName = fileName.split("\\.")[0] + "-" + author  + "." + fileName.split("\\.")[fileName.split("\\.").length - 1];
					 }
					 String savePath = realPath + ConstCodeUtils.FILE_SPLIT + realFileName;
					 //吴道亮  2016.05.23↓ 删除
					 //String zipfile = ziprealPath + "\\" + realFileName;
					 //吴道亮  2016.05.23↑ 删除
					 if(fileName.split("\\.")[fileName.split("\\.").length - 1]. equalsIgnoreCase("xls")||fileName.split("\\.")[fileName.split("\\.").length - 1]. equalsIgnoreCase("xlsx")
							 ||fileName.split("\\.")[fileName.split("\\.").length - 1]. equalsIgnoreCase("bmp")||fileName.split("\\.")[fileName.split("\\.").length - 1].equalsIgnoreCase("jpg")){
